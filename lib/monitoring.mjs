...
export function renderMonitoringReport(
  ...
  const table = rows
    .map(
      (row) =>
        `| ${row.name} | ${row.current === null ? "Sin datos" : row.current ? "Operativo" : "Con falla"} | ${row.uptime.toFixed(2)}% | ${row.failedChecks} |`
    )
    .join("\n");
...
| Control | Estado actual | Disponibilidad del período | Fallas recientes |
|---|---:|---:|---:|
...
<body><main>
  ...
  <section>
    ...
    <div class="table-wrap"><table>
      <thead><tr><th>Control</th><th>Estado actual</th><th>Disponibilidad</th><th>Fallas recientes</th></tr></thead>
