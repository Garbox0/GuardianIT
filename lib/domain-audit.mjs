...
export function assessDomain(input) {
  ...
  const redirectsToHttps =
    [301, 302, 307, 308].includes(input.httpStatus) &&
    String(input.httpLocation || "").toLowerCase().startsWith("https://");
  findings.push(
    redirectsToHttps
      ? finding("Redirección a HTTPS", "Correcta", "Informativa", `HTTP ${input.httpStatus} hacia ${input.httpLocation}.`, "Mantener la redirección.")
      : finding("Redirección a HTTPS", "No verificada", "Media", "La entrada por HTTP no redirigió de forma comprobable a HTTPS.", "Redirigir todo el tráfico HTTP a HTTPS.")
  );
  ...
  findings.push(
    clickjackingProtection
      ? finding("Protección contra marcos", "Presente", "Informativa", "Se observó X-Frame-Options o frame-ancestors.", "Mantener y comprobar las páginas que deban embeberse.")
      : finding("Protección contra marcos", "Ausente", "Media", "No se observó X-Frame-Options ni frame-ancestors.", "Definir una política para reducir el riesgo de clickjacking.")
  );
